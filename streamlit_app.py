...
st.markdown('<div class="slabel">Batch mode (CSV)</div>', unsafe_allow_html=True)

with st.expander("📦 Open batch processing", expanded=False):

    file = st.file_uploader("Upload CSV file", type=["csv"])

    column_name = st.text_input("Column name", value="text")
    ...
    if file and run_batch:
        ...
        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zipf:
            ...
            for i, row in df.iterrows():
                ...
                text_input = str(row[column_name])
